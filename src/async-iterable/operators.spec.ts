...
test("async-iterable/operators", async test => {
	...
	test.test("tap :: ((T, Number) -> Unit) -> AsyncIterable T -> AsyncIterable T", async test => {
		...
		const output = Operators.tap((value: string, index: number) => tapped.push({ value, index }))(Generators.from(input));
	...
	test.test("tap :: ((T, Number) -> Promise Unit) -> AsyncIterable T -> AsyncIterable T", async test => {
		const input = ["a", "b", "c", "f"];
		const tapped: { value: string; index: number }[] = [];
		const output = Operators.tap(async (value: string, index: number) => tapped.push({ value, index }))(Generators.from(input));
	...
	test.test("map :: ((T, Number) -> U) -> AsyncIterable T -> AsyncIterable U", async test => {
		const result = Operators.map((value: string, index) => ({ value: value + index, index }))(Generators.from(["a", "b", "c"]));
	...
	test.test("map :: ((T, Number) -> Promise U) -> AsyncIterable T -> AsyncIterable U", async test => {
		const result = Operators.map(async (value: string, index) => ({ value: value + index, index }))(Generators.from(["a", "b", "c"]));
		test.deepEqual(await Operators.toArray(result), [{ value: "a0", index: 0 }, { value: "b1", index: 1 }, { value: "c2", index: 2 }]);
		test.end();
	});
	test.test("flatMap :: ((T, Number) -> Iterable U) -> AsyncIterable T -> AsyncIterable U", async test => {
		const result = Operators.flatMap((value: string, index) => [index, { value: value + index, index }])(Generators.from(["a", "b", "c"]));
	...
	test.test("flatMap :: ((T, Number) -> AsyncIterable U) -> AsyncIterable T -> AsyncIterable U", async test => {
		const result = Operators.flatMap((value: string, index) => Generators.from([index, { value: value + index, index }]))(Generators.from(["a", "b", "c"]));
		test.deepEqual(await Operators.toArray(result), [0, { value: "a0", index: 0 }, 1, { value: "b1", index: 1 }, 2, { value: "c2", index: 2 }]);
		test.end();
	});
	test.test("filter :: ((T, Number) -> Boolean) -> AsyncIterable T -> AsyncIterable T", async test => {
		const result = Operators.filter((value: string, index) => value.startsWith("a") && index % 2 === 0)(Generators.from(["aa", "ab", "ba", "az", "ae"]));
	...
	test.test("filter :: ((T, Number) -> Promise Boolean) -> AsyncIterable T -> AsyncIterable T", async test => {
		const result = Operators.filter(async (value: string, index) => value.startsWith("a") && index % 2 === 0)(Generators.from(["aa", "ab", "ba", "az", "ae"]));
		test.deepEqual(await Operators.toArray(result), ["aa", "ae"]);
		test.end();
	});
	test.test("reduce :: ((U, T, Number) -> (AsyncIterable T, U) -> Promise U", async test => {
		const result = await Operators.reduce((prev: number, curr: string, index) => prev + parseInt(curr) + index, 100)(Generators.from(["10", "20", "30"]));
		test.equal(result, 163);
		test.end();
	});
	test.test("reduce :: ((U, T, Number) -> (AsyncIterable T, Promise U) -> Promise U", async test => {
		const result = await Operators.reduce(async (prev: number, curr: string, index) => prev + parseInt(curr) + index, 100)(Generators.from(["10", "20", "30"]));
		test.equal(result, 163);
		test.end();
	});
	test.test("scan :: ((U, T, Number) -> U) -> AsyncIterable T -> AsyncIterable U", async test => {
		const result = Operators.scan((prev: number, curr: string, index) => prev + parseInt(curr) + index, 100)(Generators.from(["10", "20", "30"]));
	...
	test.test("scan :: ((U, T, Number) -> Promise U) -> AsyncIterable T -> AsyncIterable U", async test => {
		const result = Operators.scan(async (prev: number, curr: string, index) => prev + parseInt(curr) + index, 100)(Generators.from(["10", "20", "30"]));
		test.deepEqual(await Operators.toArray(result), [110, 131, 163]);
		test.end();
	});
	test.test("fold :: ((T, T, Number) -> T) -> AsyncIterable T -> AsyncIterable T", async test => {
		const result = await Operators.fold((prev: string, curr: string, index) => (parseInt(prev) + parseInt(curr) + index).toString())(Generators.from(["10", "20", "30"]));
	...
	test.test("takeWhile :: ((T, Number) -> Boolean) -> AsyncIterable T -> AsyncIterable T", async test => {
		const result = Operators.takeWhile((value: number, index) => value < 15 && index < 3)(Generators.from([10, 13, 15, 12, 18]));
	...
	test.test("takeWhile :: ((T, Number) -> Promise Boolean) -> AsyncIterable T -> AsyncIterable T", async test => {
		const result = Operators.takeWhile(async (value: number, index) => value < 15 && index < 3)(Generators.from([10, 13, 15, 12, 18]));
		test.deepEqual(await Operators.toArray(result), [10, 13]);
		test.end();
	});
	test.test("takeUntil :: ((T, Number) -> Boolean) -> AsyncIterable T -> AsyncIterable T", async test => {
		const result = Operators.takeUntil((value: number, index) => value > 15 || index > 2)(Generators.from([10, 13, 15, 12, 18]));
	...
	test.test("takeUntil :: ((T, Number) -> Promise Boolean) -> AsyncIterable T -> AsyncIterable T", async test => {
		const result = Operators.takeUntil(async (value: number, index) => value > 15 || index > 2)(Generators.from([10, 13, 15, 12, 18]));
		test.deepEqual(await Operators.toArray(result), [10, 13, 15]);
		test.end();
	});

	test.test("skipWhile :: ((T, Number) -> Boolean) -> AsyncIterable T -> AsyncIterable T", async test => {
		const result = Operators.skipWhile((value: number, index) => value < 15 && index < 3)(Generators.from([10, 13, 15, 12, 18]));
	...
	test.test("skipWhile :: ((T, Number) -> Promise Boolean) -> AsyncIterable T -> AsyncIterable T", async test => {
		const result = Operators.skipWhile(async (value: number, index) => value < 15 && index < 3)(Generators.from([10, 13, 15, 12, 18]));
		test.deepEqual(await Operators.toArray(result), [15, 12, 18]);
		test.end();
	});
	test.test("skipUntil :: ((T, Number) -> Boolean) -> AsyncIterable T -> AsyncIterable T", async test => {
		const result = Operators.skipUntil((value: number, index) => value > 15 || index > 2)(Generators.from([10, 13, 15, 12, 18]));
	...
	test.test("skipUntil :: ((T, Number) -> Promise Boolean) -> AsyncIterable T -> AsyncIterable T", async test => {
		const result = Operators.skipUntil(async (value: number, index) => value > 15 || index > 2)(Generators.from([10, 13, 15, 12, 18]));
		test.deepEqual(await Operators.toArray(result), [12, 18]);
		test.end();
	});
	...
	test.test("some :: ((T, Number) -> Boolean) -> AsyncIterable T -> Boolean", async test => {
		const trueResult = await Operators.some((x: number, i) => x + i === 10)(Generators.from([1, 9, 3, 2]));
		const falseResult = await Operators.some((x: number, i) => x + i === 10)(Generators.from([1, 8, 3, 2]));
		test.true(trueResult);
		test.false(falseResult);
		test.end();
	});
	test.test("some :: ((T, Number) -> Promise Boolean) -> AsyncIterable T -> Boolean", async test => {
		const trueResult = await Operators.some(async (x: number, i) => x + i === 10)(Generators.from([1, 9, 3, 2]));
		const falseResult = await Operators.some(async (x: number, i) => x + i === 10)(Generators.from([1, 8, 3, 2]));
		test.true(trueResult);
		test.false(falseResult);
		test.end();
	});
	test.test("every :: ((T, Number) -> Boolean) -> AsyncIterable T -> Boolean", async test => {
		const trueResult = await Operators.every((x: number, i) => x + i !== 10)(Generators.from([1, 8, 3, 2]));
		const falseResult1 = await Operators.every((x: number, i) => x + i === 10)(Generators.from([1, 9, 3, 2]));
		const falseResult2 = await Operators.every((x: number, i) => x + i !== 10)(Generators.from([1, 9, 3, 2]));
		test.true(trueResult);
	...
	test.test("every :: ((T, Number) -> Promise Boolean) -> AsyncIterable T -> Boolean", async test => {
		const trueResult = await Operators.every(async (x: number, i) => x + i !== 10)(Generators.from([1, 8, 3, 2]));
		const falseResult1 = await Operators.every(async (x: number, i) => x + i === 10)(Generators.from([1, 9, 3, 2]));
		const falseResult2 = await Operators.every(async (x: number, i) => x + i !== 10)(Generators.from([1, 9, 3, 2]));
		test.true(trueResult);
		test.false(falseResult1);
		test.false(falseResult2);
		test.end();
	...
	test.test("orderBy :: (String -> Number, (Number, Number) -> Number) -> AsyncIterable T -> AsyncIterable T", async test => {
		const result = Operators.orderBy((a: string) => parseInt(a.substr(1, 1), 10))(Generators.from(["11", "28", "52", "85", "94", "13"]));
	...
	test.test("orderBy :: (Number -> String, (String, Number) -> Number) -> AsyncIterable T -> AsyncIterable T", async test => {
		const result = Operators.orderBy((a: number) => a.toString(), (a: string, b: string) => parseInt(b, 10) - parseInt(a, 10))(Generators.from([11, 38, 52, 84, 93, 22]));
	...
	test.test("count :: Iterable T -> Number", async test => {
		test.equals(await Operators.count(Generators.from([])), 0);
		test.equals(await Operators.count(Generators.from([1, 4, 5, 6, 7])), 5);
		test.end();
	});
	test.test("empty :: Iterable T -> Number", async test => {
		test.true(await Operators.empty(Generators.from([])));
		test.false(await Operators.empty(Generators.from([1, 4, 5, 6, 7])));
		test.end();
	});
